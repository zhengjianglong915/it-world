...
https://www.cnblogs.com/leesf456/p/5854461.html

另一种写法：

```<java>
public class Solution {  
    //用于对prev赋初值，如果值为true，需要对prev赋初值，如果为false，表示不需为其赋值了。  
    boolean flag = true;  
    //prev表示中序遍历节点的前一个节点值  
    int last = 0;  
    ...
    public boolean isBSTree(TreeNode root) {  
        if(root == null)  
            return true;  
        return isBSTree(root.left) && compare(root.val) && isBSTree(root.right);  
    }  
